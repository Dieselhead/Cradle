...
{
public:
	Meshi(const CradleD3D*);
	~Meshi();
	// Vertices
	CradleVertexDeclarations::VertexPosTex* Vertices;
...
Meshi::Meshi(const CradleD3D* cd3d)
{
	/*
	// Create vertices
	Vertices = new VertexPos[6];

	Vertices[0] = VertexPos(DirectX::XMFLOAT3(-0.5f, 0.5f, 0.5f), DirectX::XMFLOAT2(0.0f, 0.0f));
	Vertices[1] = VertexPos(DirectX::XMFLOAT3(0.5f, 0.5f, 0.5f), DirectX::XMFLOAT2(1.0f, 0.0f));
	Vertices[2] = VertexPos(DirectX::XMFLOAT3(0.5f, -0.5f, 0.5f), DirectX::XMFLOAT2(1.0f, 1.0f));

	Vertices[3] = VertexPos(DirectX::XMFLOAT3(0.5f, -0.5f, 0.5f), DirectX::XMFLOAT2(1.0f, 1.0f));
	Vertices[4] = VertexPos(DirectX::XMFLOAT3(-0.5f, -0.5f, 0.5f), DirectX::XMFLOAT2(0.0f, 1.0f));
	Vertices[5] = VertexPos(DirectX::XMFLOAT3(-0.5f, 0.5f, 0.5f), DirectX::XMFLOAT2(0.0f, 0.0f));

	VertexTypeSize = sizeof(VertexPos);
	...
	bufferDesc.ByteWidth = sizeof(VertexPos) * 6;
	...
	HRESULT hr = E_FAIL;
	hr = cd3d->m_d3dDevice->CreateBuffer(&bufferDesc, &subData, &Buffer);
	...
	hr = cd3d->m_d3dDevice->CreateInputLayout(vertexLayout
		, totalLayoutElements
		, Buffer->GetBufferPointer()	// VS Shader Blob -> GetBufferPointer()
		, Buffer->GetBufferSize()		// VS Shader Blob -> GetBufferSize()
		, &InputLayout);
