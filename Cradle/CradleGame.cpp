...
int CradleGame::Start()
{
	...
	*/
	...
	/*
	{
	(CradleMesh*)m_Resources->GetMesh(L"cube"),
	(CradleMesh*)m_Resources->GetMesh(L"cube"),
	(CradleMesh*)m_Resources->GetMesh(L"cube")
	};
	*/
	...
	{
		int j = rand();
		double r = (double)j / (RAND_MAX + 1);

		m_Actor[i].m_Offset.x = ((float)i - ((float)m_numActors * 0.5f))* 5.0f;
		//m_Actor[i].m_Offset.y = (i % 50) * 1;
...
void CradleGame::Render()
{
	...
	*/
	...
	// DRAW INSTANCED
	//int num = ((CradleMesh*)m_Resources->GetMesh(m_meshToUse))->GetNumVertices();
	
	//m_cd3d->m_d3dContext->DrawInstanced(num, m_numActors, 0, 0);

	// m_cd3d->m_d3dContext->Draw(num, 0); 
	...
	// m_cd3d->m_d3dContext->DrawIndexed(num, 0, 0);
	...
	vb = ((CradleMesh*)m_Resources->GetMesh(L"cylinder"))->GetVertexBuffer();

	m_cd3d->m_d3dContext->IASetVertexBuffers(0, 1, &vb, &stride, &offset);
	num = ((CradleMesh*)m_Resources->GetMesh(L"cylinder"))->GetNumVertices();
	...
	vb = ((CradleMesh*)m_Resources->GetMesh(L"cone"))->GetVertexBuffer();

	m_cd3d->m_d3dContext->IASetVertexBuffers(0, 1, &vb, &stride, &offset);
	num = ((CradleMesh*)m_Resources->GetMesh(L"cone"))->GetNumVertices();
...
int CradleGame::Update()
{
	...
	if (m_timer->GetTotal() - lasttime > 1.40f)
	{
		for (int i = 0; i < 4; i++)
		{
			for (int j = 0; j < 4; j++)
			{
				// wprintf_s(L"Matrix[%d][%d] = %f\n", i, j, m_Actor[0].m_mWorld.m[i][j]);
			}
		}

		lasttime = m_timer->GetTotal();
	}
